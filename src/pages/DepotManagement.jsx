...
function ContainerManagement() {
  ...
  const handleAddFormOpenImages = async (type) => {
    const containerNo = addContainerNo
    if (!containerNo) { message.warning('Nhập Container No trước'); return }
    const label = type === 'in' ? 'Hình In' : 'Hình SC'
    const handles = type === 'in' ? folderInHandlesRef.current : folderSCHandlesRef.current
    if (!handles.length) { message.warning(`Chưa chọn folder ${label.toLowerCase()}`); return }
    setImgPreviewTitle(`${containerNo} - ${label}`)
    setImgPreviewLoading(true)
    setImgPreviewOpen(true)
        ...
        if (!curSL || !curSize) {
          try {
            ...
            if (!curSL) {
              ...
              const slFromDepot = results[1].status === 'fulfilled' && results[1].value.data?.find(d => d.shippingLine)?.shippingLine
              if (slFromLock) form.setFieldValue('shippingLine', slFromLock)
              else if (slFromDepot) form.setFieldValue('shippingLine', slFromDepot)
            }
            if (!curSize) {
              const szFromLock = results[0].status === 'fulfilled' && results[0].value.data?.find(d => d.size)?.size
              const szFromDepot = results[1].status === 'fulfilled' && results[1].value.data?.find(d => d.size)?.size
              if (szFromLock) form.setFieldValue('size', szFromLock)
              else if (szFromDepot) form.setFieldValue('size', szFromDepot)
            }
          } catch {}
        }
  ...
  const handleCreateLock = async () => {
    try {
      ...
      await api.post('/locks', { date: lockDate, shift: lockShift, items, depotIds, type: 'depot' })
    ...
    <div>
      <Card className="mb-4">
        ...
        <Row gutter={[8, 8]} align="middle" style={{ marginTop: 8 }}>
          ...
          <Col xs={12} sm={8} md={2}>
            <Space>
              {addHinhIn && <Tag color="green" style={{ cursor: 'pointer' }} onClick={() => handleAddFormOpenImages('in')}>In</Tag>}
              {addHinhSC && <Tag color="blue" style={{ cursor: 'pointer' }} onClick={() => handleAddFormOpenImages('sc')}>SC</Tag>}
            </Space>
          </Col>
        </Row>
      </Card>
